...
try:
    ...
    debrid_resolvers = [resolver() for resolver in diamondresolve.relevant_resolvers(order_matters=True) if resolver.isUniversal()]
    ...
    if len(debrid_resolvers) == 0:
        ...
        debrid_resolvers = [resolver() for resolver in diamondresolve.relevant_resolvers(order_matters=True,include_universal=False) if 'rapidgator.net' in resolver.domains]
