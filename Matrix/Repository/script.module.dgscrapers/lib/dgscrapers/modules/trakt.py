...
"""
    Diamond Build Add-on
    ///Updated for Diamond Build///
...
V2_API_KEY = control.addon('plugin.video.fsmforkyou').getSetting('trakt.client_id')
CLIENT_SECRET = control.addon('plugin.video.fsmforkyou').getSetting('trakt.client_secret')
...
def __getTrakt(url, post=None):
    try:
        ...
        if getTraktCredentialsInfo():
            headers.update({'Authorization': 'Bearer %s' % control.addon('plugin.video.fsmforkyou').getSetting('trakt.token')})
        ...
        opost = {'client_id': V2_API_KEY, 'client_secret': CLIENT_SECRET, 'redirect_uri': REDIRECT_URI, 'grant_type': 'refresh_token', 'refresh_token': control.addon('plugin.video.fsmforkyou').getSetting('trakt.refresh')}
        ...
        control.addon('plugin.video.fsmforkyou').setSetting(id='trakt.token', value=token)
        control.addon('plugin.video.fsmforkyou').setSetting(id='trakt.refresh', value=refresh)
    ...
    try:
        if getTraktCredentialsInfo() == True:
            if control.yesnoDialog(control.lang(32511) + '[CR]' + control.lang(32512), heading='Trakt'):
                control.addon('plugin.video.fsmforkyou').setSetting(id='trakt.user', value='')
                control.addon('plugin.video.fsmforkyou').setSetting(id='trakt.authed', value='')
                control.addon('plugin.video.fsmforkyou').setSetting(id='trakt.token', value='')
                control.addon('plugin.video.fsmforkyou').setSetting(id='trakt.refresh', value='')
            raise Exception()
        ...
        print('info - ' + token)
        control.addon('plugin.video.fsmforkyou').setSetting(id='trakt.user', value=user)
        control.addon('plugin.video.fsmforkyou').setSetting(id='trakt.authed', value=authed)
        control.addon('plugin.video.fsmforkyou').setSetting(id='trakt.token', value=token)
        control.addon('plugin.video.fsmforkyou').setSetting(id='trakt.refresh', value=refresh)
        raise Exception()
    except:
...
def getTraktCredentialsInfo():
    user = control.addon('plugin.video.fsmforkyou').getSetting('trakt.user').strip()
    token = control.addon('plugin.video.fsmforkyou').getSetting('trakt.token')
    refresh = control.addon('plugin.video.fsmforkyou').getSetting('trakt.refresh')
    if (user == '' or token == '' or refresh == ''): return False
    return True


def getTraktIndicatorsInfo():
    indicators = control.addon('plugin.video.fsmforkyou').getSetting('indicators') if getTraktCredentialsInfo() == False else control.addon('plugin.video.fsmforkyou').getSetting('indicators.alt')
...
def cachesyncMovies(timeout=0):
    indicators = cache.get(syncMovies, timeout, control.addon('plugin.video.fsmforkyou').getSetting('trakt.user').strip())
    return indicators


def timeoutsyncMovies():
    timeout = cache.timeout(syncMovies, control.addon('plugin.video.fsmforkyou').getSetting('trakt.user').strip())
    return timeout
...
def cachesyncTVShows(timeout=0):
    indicators = cache.get(syncTVShows, timeout, control.addon('plugin.video.fsmforkyou').getSetting('trakt.user').strip())
    return indicators


def timeoutsyncTVShows():
    timeout = cache.timeout(syncTVShows, control.addon('plugin.video.fsmforkyou').getSetting('trakt.user').strip())
