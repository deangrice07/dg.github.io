...
DEBUGPREFIX = '[ DG DEBUG ]'
LOGPATH = xbmc.translatePath('special://logpath/')


def log(msg, level=LOGNOTICE):
    debug_enabled = control.addon('plugin.video.dg').getSetting('addon_debug')
    debug_log = control.addon('plugin.video.dg').getSetting('debug.location')

    print(DEBUGPREFIX + ' Debug Enabled?: ' + str(debug_enabled))
    print(DEBUGPREFIX + ' Debug Log?: ' + str(debug_log))

    if not control.addon('plugin.video.dg').getSetting('addon_debug') == 'true':
        return
    ...
    try:
        ...
        if not control.addon('plugin.video.dg').getSetting('debug.location') == '0':
            log_file = os.path.join(LOGPATH, 'dg.log')
