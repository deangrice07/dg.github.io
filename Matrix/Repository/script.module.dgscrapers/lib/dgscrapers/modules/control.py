...
addonObject = addon('script.module.dgscrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from dgscrapers.modules import libtools
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.dg.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.dg.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.dg.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.dg.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.dg.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.dg.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
