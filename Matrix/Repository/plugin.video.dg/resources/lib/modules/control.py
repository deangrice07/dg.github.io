...
def genreIconPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.dg').getAddonInfo('path'), 'resources', 'artwork', 'genre_media', 'icons')

def genrePosterPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.dg').getAddonInfo('path'), 'resources', 'artwork', 'genre_media', 'posters')
...
def refresh_debugReversed(): # called from service "onSettingsChanged" to clear dg.log if setting to reverse has been changed
	if homeWindow.getProperty('dg.debug.reversed') != setting('debug.reversed'):
