...
def router(params):
	...
	####################################################
		...
		if action == 'play_Item':
			...
			# if episode and control.homeWindow.getProperty('dg.isplaying.playlist') != 'true':
			...
			# rescrape = params.get('rescrape')
			# select = params.get('select')
			# if control.homeWindow.getProperty('dg.isplaying.playlist') == 'true' or enable_playnext is False or media_type == 'movie' or rescrape or select == '0':
				# try:
					# if control.homeWindow.getProperty('dg.isplaying.playlist') == 'true':
						# xbmc.log('[ plugin.video.dg ] dg.isplaying.playlist = %s' % control.homeWindow.getProperty('dg.isplaying.playlist'), 1)
					# from resources.lib.modules import sources
					# premiered = params.get('premiered')
					# meta = params.get('meta')
					# xbmc.log('[ plugin.video.dg ] control.playlist.size() = %s' % control.playlist.size(), 1)
					# xbmc.log('[ plugin.video.dg ] control.playlist.getposition() = %s' % control.playlist.getposition(), 1)
					# xbmc.log('[ plugin.video.dg ] Calling....sources.Sources().play()', 1)
					# if int(control.playlist.getposition()) == -1 or (int(control.playlist.getposition()) == (control.playlist.size() - 1)):
						# control.homeWindow.clearProperty('dg.isplaying.playlist')
					# sources.Sources().play(title, year, imdb, tmdb, tvdb, season, episode, tvshowtitle, premiered, meta, select, rescrape)
				# except:
					# import traceback
					# traceback.print_exc()
			# else:
				# try:
					...
					# if control.playlist.size() <= 1:
						# current_ep = episode
						# xbmc.log('[ plugin.video.dg ] current_ep = %s' % current_ep, 1)
					...
					# control.homeWindow.setProperty('dg.isplaying.playlist', 'true')
					# playlist_urls = [control.playlist[i].getPath() for i in range(control.playlist.size())]
					# xbmc.log('[ plugin.video.dg ] control.playlist.size() = %s' % control.playlist.size(), 1)
					# xbmc.log('[ plugin.video.dg ] playlist_urls = %s' % playlist_urls, 1)
					# xbmc.log('[ plugin.video.dg ] Calling....control.player2().play(control.playlist)', 1)
