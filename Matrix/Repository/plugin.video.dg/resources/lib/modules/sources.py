...
class sources:
    ...
    def sourcesProcessTorrents(self, torrent_sources):#adjusted FSM code
            ...
            if not local:
                ...
                for part in url.split(' , '):
                    ...
                    if not d == '':
                        ...
                        hmf = diamondresolve.HostedMediaFile(url=u, include_disabled=True, include_universal=False)
    ...
    def getConstants(self):
        ...
        oas_module_name = 'ForkYouScrapers (' + str(control.addon('script.module.dgscrapers').getSetting('package.folder')) + ' set):' \
                          if control.addon('script.module.dgscrapers').getSetting('package.folder') != 'DG' else 'DG:'

        import dgscrapers
        sourceDir1 = dgscrapers.sources()
        ...
        try:
            if scraperSetting == 'FSM Scrapers ':
                self.sourceDict = sourceDir1
                self.module_name = oas_module_name
            elif scraperSetting == 'FSM Scrapers ':
                ...
                self.sourceDict = sourceDir1
                self.module_name = oas_module_name
                control.setSetting('module.provider', 'FSM Scrapers ')
        except:
            return

        try:
            self.hostDict = diamondresolve.relevant_resolvers(order_matters=True)
