...
class indexer:
    ...
    def rootlists1(self):
            regex.clear()
            url = 'http://7of9.m3u.xyz/jen/diamondshadow/movies/pop-1ck.php'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def rootlists2(self):
            regex.clear()
            url = 'http://7of9.m3u.xyz/jen/diamondshadow/movies/free-list-1.php'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def rootlists3(self):
            regex.clear()
            url = 'http://7of9.m3u.xyz/jen/redditytmovies/main.php'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def rootlists4(self):
            regex.clear()
            url = 'http://7of9.m3u.xyz/jen/shared-list/dust-scifi.php'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def rootlists5(self):
            regex.clear()
            url = 'plugin://plugin.acme.livenet/list_channels/36'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
...
class resolver:
    ...
    def process(self, url, direct=True):
        ...
        try:
            import diamondresolve

            hmf = diamondresolve.HostedMediaFile(url=url)
