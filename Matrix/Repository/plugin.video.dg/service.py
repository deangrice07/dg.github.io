...
class VersionIsUpdateCheck:
	def run(self):
		try:
			...
			if isUpdate == 'true':
				control.homeWindow.setProperty('dg.updated', 'true')
...
class SyncTraktWatched:
	def run(self):
		control.log('[ plugin.video.dg ]  Trakt Watched Sync Service Starting (sync check every 15min)...', LOGINFO)
...
class SyncTraktProgress:
	def run(self):
		control.log('[ plugin.video.dg ]  Trakt Progress Sync Service Starting (sync check every 15min)...', LOGINFO)
...
def main():
	...
	if syncWatched:
		control.log('[ plugin.video.dg ]  Trakt Watched Sync Service Stopping...', LOGINFO)
	if syncProgress:
		control.log('[ plugin.video.dg ]  Trakt Progress Sync Service Stopping...', LOGINFO)
	if libraryService:
