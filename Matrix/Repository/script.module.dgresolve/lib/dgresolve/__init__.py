...
This module provides the main API for accessing the dgresolve features.

For most cases you probably want to use :func:`dgresolve.resolve` or
:func:`dgresolve.choose_source`.
...
from dgresolve import common
from dgresolve.hmf import HostedMediaFile
from dgresolve.resolver import DGResolve
from dgresolve.plugins.__resolve_generic__ import ResolveGeneric
from dgresolve.plugins import *  # NOQA

common.logger.log_debug('Initializing DGResolve version: %s' % common.addon_version)
MAX_SETTINGS = 75
...
def relevant_resolvers(domain=None, include_universal=None, include_popups=None, include_external=False, include_disabled=False, order_matters=False):
    ...
    classes = DGResolve.__class__.__subclasses__(DGResolve) + DGResolve.__class__.__subclasses__(ResolveGeneric)
    relevant = []
...
def resolve(web_url):
    ...
    It is usually as simple as::
        ...
        import dgresolve
        media_url = dgresolve.resolve(web_url)
    ...
    Behind the scenes, :mod:`dgresolve` will check each of the available
...
def choose_source(sources):
    ...
    Example::
        ...
        source = dgresolve.choose_source(sources)
        if source:
...
def scrape_supported(html, regex=None, host_only=False):
    """
    returns a list of links scraped from the html that are supported by dgresolve
...
def display_settings():
    """
    Opens the settings dialog for :mod:`dgresolve` and its plugins.

    This can be called from your addon to provide access to global
    :mod:`dgresolve` settings. Each resolver plugin is also capable of
    exposing settings.

    .. note::

        All changes made to these setting by the user are global and will
        affect any addon that uses :mod:`dgresolve` and its plugins.
    """
...
def _update_settings_xml():
    ...
    new_xml = [
        ...
        '\t\t<setting id="reset_cache" type="action" label="%s" action="RunPlugin(plugin://script.module.dgresolve/?mode=reset_cache)"/>' % (common.i18n('reset_function_cache')),
