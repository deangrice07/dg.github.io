"""
    Linksnappy plugin for DiamondREsolve
...
from diamondresolve.lib import kodi
from diamondresolve import common
from diamondresolve.common import i18n
from diamondresolve.resolver import DiamondResolve, ResolverError

logger = common.log_utils.Logger.get_logger(__name__)
logger.disable()

USER_AGENT = 'DiamondREsolve {0} for Kodi {1}'.format(common.addon_version, kodi.kodi_version())
...
## Provides a zip-file of a torrent, not used in diamondresolve
...
folder_name = 'diamondresolve'


# noinspection PyBroadException
class LinksnappyResolver(DiamondResolve):
    ...
    def __initiate_transfer(self, media_id, interval=5):
        ...
        if transfer_info:
            ...
            with common.kodi.ProgressDialog('DiamondREsolve Linksnappy transfer', line1, line2, line3) as pd:
            ...
            if torrent:
                ...
                if stream['status'] != 'OK':
                    ...
                    if transfer_info.get('percent') != 100:
                        ...
                        with common.kodi.ProgressDialog('DiamondREsolve Linksnappy transfer', line1, line2) as pd:
    ...
    def get_settings_xml(cls):
        ...
        xml.append(
            '<setting id="{0}_auth" type="action" label="{1}" action="RunPlugin(plugin://script.module.diamondresolve/?mode=auth_ls)" option="close"/>'.format(
                cls.__name__, i18n('auth_my_account')
            )
        )

        xml.append(
            '<setting id="{0}_reset" type="action" label="{1}" action="RunPlugin(plugin://script.module.diamondresolve/?mode=reset_ls)"/>'.format(
