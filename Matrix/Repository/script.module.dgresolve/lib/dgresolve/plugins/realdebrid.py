...
class RealDebridResolver(DiamondResolve):
    name = "Real-Debrid"
    domains = ["*"]
        ...
        try:
            ...
            if media_id.lower().startswith('magnet:'):
                ...
                if not torrent_id == "":
                    ...
                    heading = 'Diamond REsolve Real-Debrid Transfer'
    ...
    def authorize_resolver(self):
        ...
        with common.kodi.CountdownDialog('Diamond REsolve Real Debrid Authorization', line1, line2, countdown=120, interval=js_result['interval']) as cd:
    ...
    def get_settings_xml(cls):
        ...
        xml.append('<setting id="%s_auth" type="action" label="%s" action="RunPlugin(plugin://script.module.diamondresolve/?mode=auth_rd)"/>' % (cls.__name__, i18n('auth_my_account')))
        xml.append('<setting id="%s_reset" type="action" label="%s" action="RunPlugin(plugin://script.module.diamondresolve/?mode=reset_rd)"/>' % (cls.__name__, i18n('reset_my_auth')))
