...
class AllDebridResolver(DiamondResolve):
    name = "AllDebrid"
    domains = ['*']
    ...
    def __initiate_transfer(self, transfer_id, interval=5):
        try:
            ...
            if transfer_info:
                ...
                with common.kodi.ProgressDialog('DiamondREsolve AllDebrid Transfer', line1, line2, line3) as pd:
    ...
    def authorize_resolver(self):
        ...
        with common.kodi.CountdownDialog('DiamondResolve AllDebrid Authorization', line1, line2,
    ...
    def get_settings_xml(cls):
        ...
        xml.append('<setting id="{0}_auth" type="action" label="{1}" action="RunPlugin(plugin://script.module.diamondresolve/?mode=auth_ad)"/>'.format(cls.__name__, i18n('auth_my_account')))
        xml.append('<setting id="{0}_reset" type="action" label="{1}" action="RunPlugin(plugin://script.module.diamondresolve/?mode=reset_ad)"/>'.format(cls.__name__, i18n('reset_my_auth')))
