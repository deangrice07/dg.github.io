...
CLIENT_ID = '522962560'
USER_AGENT = 'DiamondREsolve for Kodi/%s' % common.addon_version
...
class PremiumizeMeResolver(DiamondResolve):
    name = "Premiumize.me"
    domains = ["*"]
    media_url = None
    ...
    def __initiate_transfer(self, media_id, interval=5):
        ...
        if transfer_info:
            ...
            with common.kodi.ProgressDialog('DiamondREsolve Premiumize Transfer', line1, line2, line3) as pd:
    ...
    def authorize_resolver(self):
        ...
        with common.kodi.CountdownDialog('Diamond REsolve Premiumize Authorization', line1, line2, countdown=120,
    ...
    def get_settings_xml(cls):
        ...
        xml.append('<setting id="%s_auth" type="action" label="%s" action="RunPlugin(plugin://script.module.diamondresolve/?mode=auth_pm)"/>' % (cls.__name__, i18n('auth_my_account')))
        xml.append('<setting id="%s_reset" type="action" label="%s" action="RunPlugin(plugin://script.module.diamondresolve/?mode=reset_pm)"/>' % (cls.__name__, i18n('reset_my_auth')))
