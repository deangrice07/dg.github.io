...
USER_AGENT = 'DiamondREsolve for Kodi/{0}'.format(common.addon_version)
FORMATS = common.VIDEO_FORMATS

api_url = 'https://debrid-link.fr/api/v2'


class DebridLinkResolver(DiamondResolve):
    name = "Debrid-Link.fr"
    domains = ["*"]
    ...
    def __initiate_transfer(self, transfer_id, interval=5):
        try:
            ...
            if transfer_info:
                ...
                with common.kodi.ProgressDialog('DiamondREsolve Debrid-Link Transfer', line1, line2, line3) as pd:
    ...
    def authorize_resolver(self):
        ...
        with common.kodi.CountdownDialog('DiamondREsolve Debrid-Link Authorization', line1, line2,
    ...
    def get_settings_xml(cls):
        ...
        xml.append('<setting id="{0}_auth" type="action" label="{1}" action="RunPlugin(plugin://script.module.diamondresolve/?mode=auth_dl)"/>'.format(cls.__name__, i18n('auth_my_account')))
        xml.append('<setting id="{0}_reset" type="action" label="{1}" action="RunPlugin(plugin://script.module.diamondresolve/?mode=reset_dl)"/>'.format(cls.__name__, i18n('reset_my_auth')))
