...
def artPath():
    ...
    if theme in ['-', '']:
        ...
        return os.path.join(
            xbmcaddon.Addon('script.dg.artwork').getAddonInfo('path'),
            'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility(
        'System.HasAddon(script.dg.artwork)') else setting('appearance.alt').lower()
    return appearance
...
def metaFile():
    if condVisibility('System.HasAddon(script.dg.metadata)'):
        return os.path.join(
            xbmcaddon.Addon('script.dg.metadata').getAddonInfo('path'),
