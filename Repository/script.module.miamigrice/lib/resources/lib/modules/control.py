...
def artPath():
    ...
    if theme in ['-', '']:
        return
    elif condVisibility('System.HasAddon(script.miamigrice.artwork)'):
        return os.path.join(
            xbmcaddon.Addon('script.miamigrice.artwork').getAddonInfo('path'),
            'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility(
        'System.HasAddon(script.miamigrice.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.miamigrice.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.miamigrice.metadata)'):
        return os.path.join(
            xbmcaddon.Addon('script.miamigrice.metadata').getAddonInfo('path'),
