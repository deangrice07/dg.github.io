...
class sources:
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=20):
        ...
        for i in range(0, 4 * timeout):
            try:
                ...
                if debrid_status:
                    d_4k_label = total_format % ('red', d_source_4k) if d_source_4k == 0 else total_format % ('magenta', d_source_4k)
                    d_1080_label = total_format % ('red', d_source_1080) if d_source_1080 == 0 else total_format % ('magenta', d_source_1080)
                    d_720_label = total_format % ('red', d_source_720) if d_source_720 == 0 else total_format % ('magenta', d_source_720)
                    d_sd_label = total_format % ('red', d_source_sd) if d_source_sd == 0 else total_format % ('magenta', d_source_sd)
                    d_total_label = total_format % ('red', d_total) if d_total == 0 else total_format % ('magenta', d_total)
	...
	if control.condVisibility('System.HasAddon(script.module.openscrapers)'):
        ...
        try:
            if xbmc.getCondVisibility('System.HasAddon(%s)' % 'script.module.liptonscrapers') and not scraperSetting == 'Open Scrapers':
                from liptonscrapers import sources
                self.sourceDict = sources()
                self.module_name = control.addon('script.module.liptonscrapers').getSetting('module.provider')                 
            else:
                from openscrapers import sources
                self.sourceDict = sources()
                self.module_name = 'Open Scrapers'
                control.setSetting('module.provider', 'Open Scrapers')
        except: return
