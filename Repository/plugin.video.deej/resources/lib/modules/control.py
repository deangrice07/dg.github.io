...
def artPath():
    theme = appearance()
    return os.path.join(xbmcaddon.Addon('plugin.video.deej').getAddonInfo('path'), 'resources', 'artwork', theme)
...
def metaFile():
    if condVisibility('System.HasAddon(script.deej.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.deej.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
