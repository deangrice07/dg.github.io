...
class sources:
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=30):
        ...
        progressDialog.create("{1} ({0} )".format(control.addonInfo('name'), self.module_name), '')
        progressDialog.update(0)
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.deej.container.items'

        self.metaProperty = 'plugin.video.deej.container.meta'

        if control.condVisibility('System.HasAddon(script.module.openscrapers)'):
           scraperSetting = control.setting('module.provider')
	
        try:
            if xbmc.getCondVisibility('System.HasAddon(%s)' % 'script.module.openscrapers') and not scraperSetting == 'deejmodule':
                from openscrapers import sources
                self.sourceDict = sources()
                self.module_name = control.addon('script.module.openscrapers').getSetting('module.provider')                 
            else:
                from deejscrapers import sources
                self.sourceDict = sources()
                self.module_name = 'deejmodule'
                control.setSetting('module.provider', 'deejmodule')
        except: return
