...
class tvMaze:
    ...
    def episodeAbsoluteNumber(self, thetvdb, season, episode):
        try:
            url = 'http://thetvdb.com/api/%s/series/%s/default/%01d/%01d' % ('QkEzRURERTdENDE1QjJFMg=='.decode('base64'), thetvdb, int(season), int(episode))
    ...
    def getTVShowTranslation(self, thetvdb, lang):
        try:
            url = 'http://thetvdb.com/api/%s/series/%s/%s.xml' % ('QkEzRURERTdENDE1QjJFMg=='.decode('base64'), thetvdb, lang)
